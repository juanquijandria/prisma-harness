...
This page is an English translation, declared as such, of the canonical Spanish block dated 2026-09-05 that lives in `docs/es/METHOD.md`. That block is the reference this repository ships, and `hooks/check-canonical-sync.sh` compares your own copies against it at every session start. If the two languages ever disagree, the Spanish block wins and this page is out of date.
...
| **Mechanical change** | Can you PROVE it changes no executable behavior, stored data, permissions, integration or deployment? If you cannot prove it, it is code | renaming a variable, moving a file, fixing a typo in a string nobody parses | **step 3 only**, run it and look. No skill |
...
| **Data to a third party** | Does a file, a list or a figure leave to someone else? | a spreadsheet for a colleague, a CRM export, a number for the CEO | **adds controls, never replaces them.** The query that produces the file is code and runs its lane; the delivery adds mandatory step 3, opening the file and looking at real rows, plus a refuter |
...
|---|---|---|---|---|
| **1** | **Plan** | an interview that sharpens the idea into shared understanding and leaves a record | **invoke NOW** `mattpocock-skills:grilling` (the human, directly, can type `/mattpocock-skills:grill-with-docs`) | the terminal starts it; the human answers decisions |
...
| 1c | only if it does NOT fit | write the destination and the path as session-sized tickets | spec and tickets (the human, directly, `/to-spec` then `/to-tickets`) | the agent |
| **2** | **Build** | run the orchestrator chain | **invoke NOW**, in order: build, then `mattpocock-skills:tdd`, then `mattpocock-skills:code-review` in clean subagents, then commit (the human, directly, `/implement`) | the agent |
