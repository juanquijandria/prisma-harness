...
- **The lint and comments gates check exactly what the push sends, or say they did not.** They read the commits. The lint gate runs the linter on the files and the configuration on disk, so it measures only when the working tree has no uncommitted changes, and otherwise prints a `WARN`. When the command runs, anywhere, a git command that can change what is committed, a commit, a switch, a checkout, a reset, a rebase, a merge, a pull, a cherry-pick, a revert, an am, an update-ref, a symbolic-ref or a commit-tree, both print a `WARN` and measure nothing, because a gate runs before the command and cannot know what each push will send. Push in a command of its own to have it measured. A git command the gate cannot read, such as one with `-C`, counts as one that can change it. A push that sends only tags or deletes a branch is measured by none of the three.
- **What the push gates cannot see.** A commit or a change of branch that the command parser does not reach, inside `$( )`, backticks, `eval`, `xargs`, `sh -c`, an alias, a wrapper such as `nice`, an `if`, `while` or `until` condition, or another program such as `npm version`. A `cd` that is not the first thing in the command, so a gate may read the repository the session stands in. A push without a refspec is read as a push of HEAD, whatever `push.default` or a configured push refspec say, and `gh pr create --head` or `gh pr ready` with a number are read as the local HEAD. A branch moved by `git branch -f` or by a fetch into it, a file marked skip-worktree, and a linter configuration that is not tracked.
