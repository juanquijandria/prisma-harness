...
- **The lint and comments gates check exactly what the push sends, or say they did not.** They read the commits, with the diff settings of the git configuration switched off. The lint gate runs the linter on the files and the configuration on disk, so it measures only when the working tree has no uncommitted changes and no untracked files, and otherwise prints a `WARN`. Every git call in the command has to be one known to leave the commits alone, push, status, log, diff, show, rev-parse, merge-base, ls-files, ls-remote, describe, remote, config, fetch, add, tag, branch, blame, grep, shortlog, rev-list, cat-file or for-each-ref. Any other, or one the gate cannot read such as one with `-C`, makes both print a `WARN` and measure nothing, because a gate runs before the command and cannot know what each push will send. Push in a command of its own to have it measured. A push without a refspec is not measured by any push gate when `push.default` is `matching` or a push refspec is configured, since then it may not send HEAD. A push that sends only tags or deletes a branch is measured by none of the three.
- **What the push gates cannot see.** A commit or a change of branch that the command parser does not reach, inside `$( )`, backticks, `eval`, `xargs`, `sh -c`, an alias, a wrapper such as `nice`, an `if`, `while` or `until` condition, or another program such as `npm version`. A `cd` that is not the first thing in the command, so a gate may read the repository the session stands in. `gh pr create --head` and `gh pr ready` with a number are read as the local HEAD. A fetch into the checked-out branch with `--update-head-ok`, replace refs, files marked skip-worktree or assume-unchanged, and line endings that `core.autocrlf` rewrites on disk.
