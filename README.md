...
- **The lint and comments gates check what the push sends.** When the command only pushes, they read the commits, and a file the push sends with uncommitted changes gets a `WARN` instead of a verdict, because the linter reads files from disk. When the same command commits first, that commit does not exist yet when the gate runs, so they read the working tree, and a commit of only part of what is edited is read whole. A push that sends only tags or deletes a branch is measured by none of the three.
