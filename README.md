...
- **The lint and comments gates check exactly what the push sends, or say they did not.** They read the commits. The lint gate runs the linter on files from disk, so a file the push sends with uncommitted changes gets a `WARN` instead of a verdict. When the same command runs, before its push, a git command that changes what is committed, a commit, a switch, a checkout, a reset, a rebase, a merge, a pull, a cherry-pick, a revert or an am, the commit that will go out does not exist yet when the gate runs, so both print a `WARN` and measure nothing. Push in a command of its own to have it measured. A git command before the push that the gate cannot read, such as one with `-C`, counts as one that changes it. A commit hidden behind an alias or `bash -c` is not seen, and the push is read as a push alone. A push that sends only tags or deletes a branch is measured by none of the three.
