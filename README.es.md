...
- **Las puertas de lint y de comentarios revisan lo que el push envía.** Cuando el comando solo empuja, leen los commits. La de lint corre el linter sobre los archivos del disco, así que un archivo del push con cambios sin commit recibe un `WARN` en vez de un veredicto. Cuando el mismo comando hace commit antes, ese commit todavía no existe cuando corre la puerta, así que las dos leen el árbol de trabajo, y un commit de solo parte de lo editado se lee entero. Un commit que el parser de comandos no ve, detrás de un alias o de `bash -c`, se lee como un push solo. Un push que solo manda tags o borra una rama no lo mide ninguna de las tres.
