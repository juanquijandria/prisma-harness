...
- **Las puertas de lint y de comentarios revisan exactamente lo que el push envía, o dicen que no lo hicieron.** Leen los commits. La de lint corre el linter sobre los archivos y la configuración del disco, así que solo mide cuando el árbol de trabajo no tiene cambios sin commit, y si no, imprime un `WARN`. Cuando el comando corre, en cualquier parte, un git que puede cambiar lo que está en los commits, un commit, un switch, un checkout, un reset, un rebase, un merge, un pull, un cherry-pick, un revert, un am, un update-ref, un symbolic-ref o un commit-tree, las dos imprimen un `WARN` y no miden nada, porque una puerta corre antes del comando y no puede saber qué enviará cada push. Para que se mida, el push va en un comando propio. Un git que la puerta no puede leer, como uno con `-C`, cuenta como uno que puede cambiar los commits. Un push que solo manda tags o borra una rama no lo mide ninguna de las tres.
- **Lo que las puertas de push no ven.** Un commit o un cambio de rama al que el parser de comandos no llega, dentro de `$( )`, de comillas invertidas, de `eval`, de `xargs`, de `sh -c`, de un alias, de un envoltorio como `nice`, de una condición `if`, `while` o `until`, o de otro programa como `npm version`. Un `cd` que no va al inicio del comando, así que una puerta puede leer el repositorio en el que está la sesión. Un push sin refspec se lee como un push de HEAD, digan lo que digan `push.default` o un refspec de push configurado, y `gh pr create --head` o `gh pr ready` con un número se leen como el HEAD local. Una rama movida con `git branch -f` o con un fetch hacia ella, un archivo marcado skip-worktree, y una configuración del linter que no está en git.
