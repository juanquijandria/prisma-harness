...
- **Las puertas de lint y de comentarios revisan lo que el push envía.** Cuando el comando solo empuja, leen los commits, y un archivo del push con cambios sin commit recibe un `WARN` en vez de un veredicto, porque el linter lee los archivos del disco. Cuando el mismo comando hace commit antes, ese commit todavía no existe cuando corre la puerta, así que leen el árbol de trabajo, y un commit de solo parte de lo editado se lee entero. Un push que solo manda tags o borra una rama no lo mide ninguna de las tres.
