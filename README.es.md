...
- **Las puertas de lint y de comentarios revisan exactamente lo que el push envía, o dicen que no lo hicieron.** Leen los commits. La de lint corre el linter sobre los archivos del disco, así que un archivo del push con cambios sin commit recibe un `WARN` en vez de un veredicto. Cuando el mismo comando corre, antes de su push, un git que cambia lo que está en los commits, un commit, un switch, un checkout, un reset, un rebase, un merge, un pull, un cherry-pick, un revert o un am, el commit que va a salir todavía no existe cuando corre la puerta, así que las dos imprimen un `WARN` y no miden nada. Para que se mida, el push va en un comando propio. Un git antes del push que la puerta no puede leer, como uno con `-C`, cuenta como uno que cambia los commits. Un commit escondido detrás de un alias o de `bash -c` no se ve, y el push se lee como un push solo. Un push que solo manda tags o borra una rama no lo mide ninguna de las tres.
