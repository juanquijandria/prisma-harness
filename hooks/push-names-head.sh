#!/bin/sh
# Prisma Harness. Reads a push command token by token, to tell whether it sends a branch, whether it can be proven to send the branch HEAD is on, and whether every git call in the command is one known to leave the commits alone. Documented in README.md, section "What is inside".
...
push_names_head() {
  ...
  if [ "$push_branch_refspecs" = "0" ]; then
    push_configuration_decides_what_is_sent && return 1
...
git_may_change_what_is_pushed() {
  ...
  while IFS= read -r git_segment; do
    ...
    is_one_of "$git_subcommand" $GIT_SUBCOMMANDS_THAT_LEAVE_COMMITS_ALONE || return 0
  done <<SEGMENTS
$git_segments
SEGMENTS
  return 1
...
unmeasured_because_git_may_change() {
  echo "WARN: this command runs a git command that is not known to leave the commits alone, so $1 cannot know what the push sends and measured nothing. Push in a command of its own to have it measured." >&2
